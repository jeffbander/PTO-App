...
def register_routes(app):
    ...
    @app.route('/export-data/<secret>')
    def export_data(secret):
        if secret != 'mswcvi-export-2026':
            return jsonify({'error': 'unauthorized'}), 403
        from models import CallOutRecord, TardinessRecord
        positions = [{'id': p.id, 'name': p.name, 'team': p.team} for p in Position.query.all()]
        members = []
        for m in TeamMember.query.all():
            members.append({
                'id': m.id, 'name': m.name, 'email': m.email, 'phone': m.phone, 'pin': m.pin,
                'position_id': m.position_id, 'position_name': m.position.name if m.position else None,
                'team': m.team, 'pto_balance_hours': float(m.pto_balance_hours or 0),
                'sick_balance_hours': float(m.sick_balance_hours or 0),
                'starting_pto_hours': float(m.starting_pto_hours or 60),
                'starting_sick_hours': float(m.starting_sick_hours or 60),
                'pto_refresh_date': str(m.pto_refresh_date) if m.pto_refresh_date else None,
                'created_at': str(m.created_at) if m.created_at else None,
            })
        managers = [{'id': mg.id, 'name': mg.name, 'email': mg.email, 'role': mg.role, 'password_hash': mg.password_hash} for mg in Manager.query.all()]
        pto_requests = []
        for r in PTORequest.query.all():
            pto_requests.append({
                'id': r.id, 'member_id': r.member_id, 'start_date': r.start_date, 'end_date': r.end_date,
                'pto_type': r.pto_type, 'status': r.status, 'manager_team': r.manager_team,
                'denial_reason': r.denial_reason, 'is_partial_day': r.is_partial_day,
                'start_time': r.start_time, 'end_time': r.end_time, 'reason': r.reason,
                'is_call_out': r.is_call_out, 'timekeeping_entered': r.timekeeping_entered,
                'coverage_arranged': r.coverage_arranged,
        ...
        pending = [{'id': pe.id, 'name': pe.name, 'email': pe.email, 'team': pe.team, 'position': pe.position, 'status': pe.status, 'submitted_at': str(pe.submitted_at) if pe.submitted_at else None} for pe in PendingEmployee.query.all()]
        tardiness = [{'id': t.id, 'member_id': t.member_id, 'date': t.date, 'minutes_late': t.minutes_late, 'reason': t.reason, 'recorded_by_id': t.recorded_by_id, 'created_at': str(t.created_at) if t.created_at else None} for t in TardinessRecord.query.all()]
        callouts = [{'id': c.id, 'member_id': c.member_id, 'pto_request_id': c.pto_request_id, 'source': c.source, 'phone_number_used': c.phone_number_used, 'verified': c.verified, 'message_text': c.message_text, 'created_at': str(c.created_at) if c.created_at else None} for c in CallOutRecord.query.all()]
        return jsonify({
            'exported_at': str(get_eastern_time()), 'positions': positions, 'team_members': members,
            'managers': managers, 'pto_requests': pto_requests, 'pending_employees': pending,
            'tardiness_records': tardiness, 'call_out_records': callouts,
            'counts': {'positions': len(positions), 'team_members': len(members), 'managers': len(managers),
                       'pto_requests': len(pto_requests), 'pending_employees': len(pending),
                       'tardiness_records': len(tardiness), 'call_out_records': len(callouts)}
        })
