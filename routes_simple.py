...
def register_routes(app):
    ...
    def export_data(secret):
        """Temporary endpoint to export all database data as JSON for migration."""
        ...
        # Export positions
        positions = [{'id': p.id, 'name': p.name, 'team': p.team}
                     for p in Position.query.all()]

        # Export team members (with position info)
        members = []
        for m in TeamMember.query.all():
            members.append({
                'id': m.id, 'name': m.name, 'email': m.email,
                'phone': m.phone, 'pin': m.pin,
                'position_id': m.position_id,
                'position_name': m.position.name if m.position else None,
                'team': m.team,
                'pto_balance_hours': float(m.pto_balance_hours or 0),
        ...
        # Export managers
        managers = []
        for mg in Manager.query.all():
            managers.append({
                'id': mg.id, 'name': mg.name, 'email': mg.email,
                'role': mg.role, 'password_hash': mg.password_hash,
            })
        ...
        for r in PTORequest.query.all():
            pto_requests.append({
                ...
                'pto_type': r.pto_type, 'status': r.status,
                'manager_team': r.manager_team,
                'denial_reason': r.denial_reason,
                'is_partial_day': r.is_partial_day,
                'start_time': r.start_time, 'end_time': r.end_time,
                'reason': r.reason, 'is_call_out': r.is_call_out,
                'timekeeping_entered': r.timekeeping_entered,
        ...
        # Export pending employees
        pending = []
        for pe in PendingEmployee.query.all():
            pending.append({
                'id': pe.id, 'name': pe.name, 'email': pe.email,
                'team': pe.team, 'position': pe.position,
                'status': pe.status,
                'submitted_at': str(pe.submitted_at) if pe.submitted_at else None,
            })

        # Export tardiness records
        tardiness = []
        for t in TardinessRecord.query.all():
            tardiness.append({
                'id': t.id, 'member_id': t.member_id,
                'date': t.date, 'minutes_late': t.minutes_late,
                'reason': t.reason,
                'recorded_by_id': t.recorded_by_id,
                'created_at': str(t.created_at) if t.created_at else None,
            })

        # Export call out records
        callouts = []
        for c in CallOutRecord.query.all():
            callouts.append({
                'id': c.id, 'member_id': c.member_id,
                'pto_request_id': c.pto_request_id,
                'source': c.source,
                'phone_number_used': c.phone_number_used,
                'verified': c.verified,
        ...
        return jsonify({
            'exported_at': str(get_eastern_time()),
            'positions': positions,
            'team_members': members,
            'managers': managers,
            'pto_requests': pto_requests,
            'pending_employees': pending,
            'tardiness_records': tardiness,
            'call_out_records': callouts,
            'counts': {
                'positions': len(positions),
                'team_members': len(members),
                'managers': len(managers),
                'pto_requests': len(pto_requests),
                'pending_employees': len(pending),
                'tardiness_records': len(tardiness),
                'call_out_records': len(callouts),
            }
        })
